...
import { AppState, AppAction, UserState, DatesState, SerializedDatesState, MiscellaneousActions } from '../types/store';
...
const dateTransform = createTransform<DatesState, SerializedDatesState>(
  (inboundState) => {
    ...
    const mappedDates = (Object.keys(dates) as Array<keyof DatesState>).reduce(
      (map: { [K in keyof DatesState]?: string | string[] | null }, dateType: keyof DatesState) => {
  ...
  (outboundState) => {
    ...
    const mappedDates = (Object.keys(dates) as Array<keyof SerializedDatesState>).reduce(
      (map: { [K in keyof DatesState]?: Date | Date[] | null }, dateType: keyof DatesState) => {
        const dateOrDates = dates[dateType];
        if (dateOrDates === null) {
          map[dateType] = dateOrDates;
        } else {
...
const migrations = {
  ...
  // add e-learning settings to DatesState
  5: (state: AppState) => ({
    ...state,
